...
namespace KingFeng.Controllers
{
    ...
    public class QingLongController : ControllerBase
    {
        ...
        public async Task<ContentResultModel> env(List<EnvModel> envs)
        {
            ...
            var url = $"{_config.QL_URL}open/envs?t={ExtensionsMethod.GetTimeStamp()}";
            ...
            {
                if (!string.IsNullOrWhiteSpace(_config.WsKeyTaskFullName))
                {
                    ClientRequest = new RestRequest();
                    //执行wskey转换
                    await task(_config.WsKeyTaskFullName, _config.SecretKey);
                }
        ...
        public async Task<ContentResultModel> updateEnv([Required] string uid, [Required] string wskey, string remarks = "")
        {
            ...
            var envs = await this.envs(_config.SecretKey);
        ...
        public async Task<ContentResultModel> task(string taskName, [FromQuery] string key)
        {
            ...
            if (taskName == "ws")
            {
                taskName = _config.WsKeyTaskFullName;
            }

            Client.BaseUrl = new Uri($"{_config.QL_URL}open/crons?searchValue={taskName}&t={ExtensionsMethod.GetTimeStamp()}");
            ClientRequest.Method = Method.GET;
            var Content = ((await Client.ExecuteAsync(ClientRequest)).Content).ToJobj();
            if (Content.ContainsKey("code") || Content["code"].ToString() == "200")
            {
            ...
            //Body.Add(task_id);
            var url = $"{_config.QL_URL}open/crons/run?t={ExtensionsMethod.GetTimeStamp()}";
        ...
        public async Task<ContentResultModel> log(string taskName, [FromQuery] string key)
        {
            ...
            var url = $"{_config.QL_URL}open/crons/{TaskID[0]}/log?t={ExtensionsMethod.GetTimeStamp()}";
        ...
        {
            if (_config.QL_URL != null && _config.QL_Client_ID != null && _config.QL_Client_Secret != null)
            {
                var url = $"{_config.QL_URL}open/auth/token?client_id={_config.QL_Client_ID}&client_secret={_config.QL_Client_Secret}";
                Client.BaseUrl = new Uri(url);
                ClientRequest.Method = Method.GET;
